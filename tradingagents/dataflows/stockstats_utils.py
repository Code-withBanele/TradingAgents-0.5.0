...
def _cache_is_fresh(data_file, curr_date_dt, now) -> bool:
    ...
    Keep the day guard and the age guard in the same epoch-based reference frame
    so a same-day cache is judged by actual age, not by a local-time drift from
    the machine timezone.
    """
    written_epoch = os.path.getmtime(data_file)
    now_epoch = pd.Timestamp(now).timestamp()
    written_date = pd.Timestamp.utcfromtimestamp(written_epoch).date()
    now_date = pd.Timestamp.utcfromtimestamp(now_epoch).date()

    if written_date != now_date:
        return False
    if curr_date_dt.date() < now_date:
        return True
    return (now_epoch - written_epoch) <= OHLCV_CACHE_TTL_SECONDS
