...
class TestSignalProcessor:
    ...
    def test_empty_or_non_string_signal_is_review(self):
        sp = SignalProcessor()
        assert sp.process_signal(None) == RATING_REVIEW
        assert sp.process_signal(123) == RATING_REVIEW
        assert sp.process_signal([]) == RATING_REVIEW
...
@pytest.mark.unit
class TestPhase3SafeRouting:
    def test_conditional_logic_handles_empty_message_queue(self):
    ...
    def test_conditional_logic_defaults_to_safe_debate_paths(self):
        from tradingagents.graph.conditional_logic import ConditionalLogic
